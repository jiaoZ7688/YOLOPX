...
def parse_args():
    ...
    parser.add_argument('--logDir',
                        ...
                        type=str,
                        default='runs/')
    parser.add_argument('--weights', nargs='+', type=str, default='/project/zhanjiao/YOLOP/yolopx/weights/epoch-195.pth', help='model.pth path(s)')
...
def main():
    ...
    da_segment_results,ll_segment_results,detect_results, maps, times = validate(
    ...
    msg =   'Driving area Segment: Acc({da_seg_acc:.3f})    IOU ({da_seg_iou:.3f})    mIOU({da_seg_miou:.3f})\n' \
                      ...
                      'Time: inference({t_inf:.3f}s/frame)  nms({t_nms:.3f}s/frame)'.format(
